...
class Lotto:
    ...
    def init(self):
        ...
        while len(self.win) < 6:
            self.win.add(random.randrange(1, 46)) # 1~45 사이의 숫자를 붕족 없이 입력.
            # 집합 자료형이므로 알아서 중복 제외

        while True:
            n = random.randrange(1, 46)
            if not (n in self.win): # 이미 뽑은 6개의 당첨번호와 다름 +1 번호를 받아야함.
                self.bonus.add(n)
                break
    ...
    def insert(self):
        ...
        while len(self.myNum) < 6:
            ...
            if (n <= 0) or (n >= 46) or (n in self.myNum): # 1~45를 벗어나거나 중복된것 제외
                print("=> 중복된 번호를 넣거나 잘못된 번호를 넣었습니다. 다시 입력해주세요.")
                continue
            self.myNum.add(n) # 입력 넣기
    ...
    def match(self):
        ...
        if matchingNum == 6:
            ...
            print("=> [2등] 5개 + bonum 가 맞았습니다")
            print("=> 세금 뗴고 6000만원입니다.")
        else:
            print("=> 꽝입니다. 일확천금을 노리지 말고 적금 할까요?")
